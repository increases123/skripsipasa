...
$no_hitung = 1;
$sql_hitung = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tgroup");
while($data_hitung = mysqli_fetch_array($sql_hitung)){
	$id_hitung = $data_hitung[groupId];		
	$hasil_hitung = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tdescription, tgroup WHERE tdescription.groupId = '$id_hitung' AND tdescription.groupId = tgroup.groupId ORDER BY tgroup.groupId");
	$i_hitung = 1;
	...
	mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO tcompany(companyId,companyName,companyAddress,companyPhoneHP,dateSurvey,suggestion,product)
	...
	while($data = mysqli_fetch_array($sql)){
		...
		while ($r = mysqli_fetch_array($hasil)){
			...
			// echo "$i $asfa<br>";
			if ($asfa == 'A'){
				mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO tanswer (descriptionId,groupId,companyId,jawaban,jawabanA,jawabanB,jawabanC,jawabanD,jawabanE) 
